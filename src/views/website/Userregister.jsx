						...
						<section className="min-h-screen flex items-stretch text-white ">
								...
								<div className="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0" style={{ backgroundColor: '#161616' }}>
										...
										<div className="w-full py-6 z-20">
												...
												<form onSubmit={handleSubmit(onSubmit)} className="sm:w-2/3 w-full px-4 grid grid-cols-2 lg:px-0 mx-auto">
														<div className="pb-2 pt-4">
																<input {...register('firstname', { required: true, maxLength: 15 , minLength:3 })} type="text"  placeholder="First Name" className="block w-full p-4 text-lg rounded-sm bg-black" />
																...
																<input {...register('lastname', { required: true, maxLength: 15 , minLength:3 })} type="text" placeholder="Last Name" className="block w-full p-4 text-lg rounded-sm bg-black" />
														</div>

														<div className="pb-2 pt-4">
																<input type="text" {...register('username', { required: true, maxLength: 15 , minLength:3 })} placeholder="Username" className="block w-full p-4 text-lg rounded-sm bg-black" />
																{errors.username &&  <p className="mt-2 text-red-500 text-xs italic">{ERROR_MESSAGES[errors.username.type]}</p>}
														</div>
														<div className="pb-2 pt-4">
																<input {...register('email', { required: true })} type="email"   className="block w-full p-4 text-lg rounded-sm bg-black" />
																{errors.email &&  <p className="mt-2 text-red-500 text-xs italic">{ERROR_MESSAGES[errors.email.type]}</p>}
														</div>
														<div className="pb-2 pt-4">
																<input {...register('password', { required: true, maxLength: 15 , minLength:3 })} className="block w-full p-4 text-lg rounded-sm bg-black" type="password" />
																{errors.password &&  <p className="mt-2 text-red-500 text-xs italic">{ERROR_MESSAGES[errors.password.type]}</p>}
														</div>
														<div className="pb-2 pt-4">
																<input {...register('confirm_password', { required: true, maxLength: 15 , minLength:3 })} className="block w-full p-4 text-lg rounded-sm bg-black" type="password" />
												...
												<div className='w-96 flex mx-auto justify-center items-center'>
														...
														<div className="px-4 pb-2 pt-4">
																<button type="submit" className="uppercase block w-full p-4 text-lg rounded-full text-white bg-indigo-500 hover:bg-indigo-600 focus:outline-none">register now</button>
														</div>
												</div>
												</form>
										</div>
